...
function registerCreateTileHook() {
  warpgate.event.watch(
    CardTilesConstants.Events.WG_CREATE_CARD_TILE_EVENT,
    onCardTileCreateEvent,
    isCurrentUserFirstGm
  );
}
...
function onCanvasDrop(event) {
  ...
  const globalPosition = canvas.stage.worldTransform.applyInverse({
    x: event.x,
    y: event.y,
  });

  /*
  Change for V10 and use of UUID 
  */
  let cardsId = eventData.uuid.match(/(?<=Cards\.).*?(?=\.Card)/)[0];
  let cardId = eventData.uuid.match(/(?<=Card\.).*$/)[0];

  const cardCollection = game.cards.get(cardsId);
  const card = cardCollection.cards.get(cardId);

  const cardTileEventData = {
    sceneID: game.user.viewedScene,
    cardCollectionID: cardCollection._id,
    cardID: card._id,
    x: globalPosition.x,
    y: globalPosition.y,
  };

  warpgate.event.notify(
    CardTilesConstants.Events.WG_CREATE_CARD_TILE_EVENT,
    cardTileEventData
  );
}
...
async function onCardTileCreateEvent(cardTileEventData) {
  ...
  const shouldPassToBoard =
    !isBoardStack(cardCollection) &&
    game.settings.get(
      CardTilesConstants.MODULE_NAME,
      CardTilesConstants.Settings.PASS_CARDS_TO_BOARD_STACK
    );
  const isDrawnFromDeck = card.drawn && cardCollection.type === "deck";
  const dontDrawFromDeck =
    game.settings.get(
      CardTilesConstants.MODULE_NAME,
      CardTilesConstants.Settings.DEAL_AFTER_DRAWN_FROM_DECK_NAME
    ) === false;
  if (dontDrawFromDeck && isDrawnFromDeck) {
    ui.notifications.warn(
      game.i18n.localize("CardTiles.Notifications.Warnings.CardAlreadyDrawn")
    );
...
async function createCardTile(cardEventData) {
  ...
  const scaling =
    game.settings.get(
      CardTilesConstants.MODULE_NAME,
      CardTilesConstants.Settings.SCALING_NAME
    ) || 1.0;
...
function buildFacesFiles(card) {
  ...
  return allFaces.map((face) => {
    return { id: randomID(16), name: face.img, selected: false };
  });
}

function getDefaultWidth(cardCollection) {
  return (
    cardCollection.width ||
    game.settings.get(
      CardTilesConstants.MODULE_NAME,
      CardTilesConstants.Settings.DEFAULT_WIDTH_NAME
    ) ||
    100
  );
}
...
function getDefaultHeight(cardCollection) {
  return (
    cardCollection.heigh ||
    game.settings.get(
      CardTilesConstants.MODULE_NAME,
      CardTilesConstants.Settings.DEFAULT_HEIGHT_NAME
    ) ||
    100
  );
}
...
function tryGetBoardStack() {
  const destinationId = game.settings.get(
    CardTilesConstants.MODULE_NAME,
    CardTilesConstants.Settings.BOARD_STACK_NAME
  );
  return game.cards.has(destinationId)
    ? game.cards.get(destinationId)
    : undefined;
}
